...
class Database {
	...
	function __construct(){
		...
		if( $DATABASE['status'] ){
			try{		
				$this->con = new mysqli( $DATABASE['hostname'], $DATABASE['username'], $DATABASE['password'], $DATABASE['dbname'] );
			}
	...
	public function select(string ...$e ){
		$this->_select = "SELECT ";
		foreach ( $e as $x ) {
	...
	public function get( $e='' ){
		if( ( $e == '' OR $e == NULL ) AND ( $this->_from == '' OR $this->_from == NULL) ){
			...
			if( isset( $this->_where ) ){ $query .= str_replace(",", " AND", $this->_where); }
			return $this->con->query( $query );
		}
	}
	public function query( string $e ){
	...
	public function insert( string $table_name = NULL, array $e = array() ){
